...
if fc-list 2>/dev/null | grep -qi "MesloLGL.*Nerd"; then
    ...
    # Check the extractor *before* pulling ~100 MB over the wire. Without this
    # the download succeeded, unzip failed, `set -e` aborted before the cleanup
    # below, and the orphaned zip was re-downloaded on every start.
    if ! command -v unzip >/dev/null 2>&1; then
        echo -e "\033[31m✘\033[0m \033[1munzip\033[0m not found — cannot unpack the font archive."
        echo -e "    Install it (\033[2mapt install unzip\033[0m) and re-run this script."
        exit 1
    fi
    ...
    mkdir -p "$FONT_DIR"
    cd "$FONT_DIR"
    # Remove the partial download on *any* exit path, including `set -e` aborts.
    trap 'rm -f "$FONT_DIR/Meslo.zip"' EXIT
