...
```sh
bash <(curl -fsSL https://raw.githubusercontent.com/iskandarputra/zyshell/main/setup-font.sh)
```
...
[![Architecture](https://img.shields.io/badge/arch-amd64-845ec2.svg)](#installation)
...
**Coming from Nushell?** The pipeline vocabulary is shared on purpose. `where`, `each`, `select`, `sort-by`, `group-by`, `first`, `flatten`, `from-json`, `to-toml` are all there with the same shape. zy adds the POSIX surface, the prompt, the file manager, and the single self-contained binary.
...
A productive terminal usually means a shell plus five to ten tools stitched together with a plugin manager and a dotfiles repo. zy ships the equivalent of all of them in one binary, already configured to work together from the first prompt.
...
| **yazi**, **ranger**, **lf** | The `explore` command. Miller-columns layout, sixel and kitty image preview, PDF/PostScript/video thumbnails, archive browsing and creation in place, tree-wide grep into a navigable pane, regex bulk-rename with live preview, a syntax-highlighted diff viewer, remappable keys, mouse and visual mode. |
| **jq**, **yq**, **dasel** | `from-json`, `to-json`, `from-yaml`, `to-toml`, `from-xml`, `from-csv`, plus `get`, `select`, `where`, `sort-by` against any of them. |
| **bat** | Syntax-highlighted file preview in `explore` and the `fff-preview` helper. |
| **ripgrep** | `fff-grep`. Runs in-process with no fork per query, so warm repeat searches stay fast; a single cold query is comparable to `rg`. |
...
| **zsh-autosuggestions**, **zsh-syntax-highlighting** | Inline syntax highlighting and history-aware suggestions in the line editor. |
| **gh** for PR status in prompt | A `github_pr` module that surfaces the PR for the current branch as a clickable chip. |
| **sqlite3** CLI for one-offs | `query-db`, `stor-open`, `stor-create`, and `schema` builtins over an in-memory SQLite store. |
...
| A separate **task runner** | A `task` builtin that reads `[task-name]` sections from a `.zytasks` file, with `depends`-based ordering. |
...
A built-in file manager. Three-pane Miller columns: a left sidebar with bookmarks, pinned folders, mounted disks (each with a usage chip), and a frecency list; the focused directory in the middle; a live preview on the right. The preview syntax-highlights code, renders images via sixel or kitty graphics, and turns PDFs, PostScript, and video into thumbnails when `poppler-utils`, `ghostscript`, or `ffmpeg` are present (text fallback otherwise). Press `,` to cycle detail columns, `I` for a file-info card (owner, permissions, size, times, architecture, SHA-256), and `s` to pick a sort. Listings sort naturally, the `/` filter is fuzzy, and the cursor is remembered per directory. No helper file manager on `$PATH`.

![zy explore TUI](assets/4-zyshell-explore.png)

Press `L` to grep the whole tree into a navigable results pane: hits group under each file with a `line:col` gutter, Enter opens a hit at its line in `$EDITOR`, Space marks hits, `Y` copies paths, `H` exports an HTML report. Scope with leading chips like `ext:c path:src !path:vendor TODO`. `Ctrl-R` opens a regex bulk-rename with a live before/after preview: PCRE2 `s/pat/repl/flags` with `$1` backrefs, upper/lower case escapes, `/g` and `/i`, and a `{n}` counter, applied through collision-safe staging. `c` compresses the selection to `.zip` and `C` to `.tar.zst`, alongside in-place archive browsing and `x` extract. Keys are remappable through an `[explore.keys]` table in `~/.zy/config.toml`.

Press `H` on a file and zy renders it as a standalone HTML page in your `$BROWSER`. A real HTML render through the bundled exporter, not a half-hearted text recolor.

![zy explore HTML preview](assets/8-zyshell-explore-html.png)

For files with uncommitted changes, the HTML view becomes a side-by-side diff against the version on disk, with a file-tree sidebar over the whole changeset. To stay in the terminal, press `\` in the file viewer for an in-terminal diff: syntax-highlighted, `n`/`N` to jump between changes, merge-conflict markers highlighted, and `w` to wrap long lines.
...
A policy engine loads rules from `/etc/zy/policy.conf` (via `--policy` at launch or the `policy` builtin) and lets administrators express which commands run in which contexts. An audit ledger records every execution with cwd, exit status, and duration, written into a SHA-256 hash chain you can inspect, export, or verify. Day to day they stay out of the way. On the day you need them, the record is already there.
...
zy is a single C binary built with `-Wall -Wextra -Werror` and link-time optimization. The runtime carries no embedded language interpreter, no plugin manager, and no "install these prerequisites first" step. Around 205,000 lines of source, one `.deb` to install, one `apt remove` to undo.
...
Counts below come straight from the source tree, checked by validators in the build, not from a marketing slide.

| | |
|---|---|
| Source | ~205,000 lines of C across 870+ files |
| Builtin commands | 533 (enforced by `tests/validation/check_builtin_count.sh`) |
...
| Architecture shipped | amd64 (the published `.deb`); also builds from source on arm64 |
...
[Releases page](https://github.com/iskandarputra/zyshell/releases/latest), then:

```bash
sudo apt install ./zy_*_amd64.deb
```

`apt` pulls in the runtime libraries automatically, and recommends `git` so the
prompt's git modules and explore's status column work out of the box. Optional tools
(`chafa`, `poppler-utils`, `ffmpeg`, `ghostscript`, `xdg-utils`, `neovim`,
`ripgrep`) are listed as Suggests; `apt show zy` surfaces them, the first-run wizard
offers the preview ones, and zy degrades gracefully without each. To upgrade later,
download the new release's `.deb` and run the same command.

<details>
<summary>Verify the download first?</summary>

Each release ships a `SHA256SUMS` (and, when signed, a `SHA256SUMS.minisig`). The
public key lives at [zy-release.pub](zy-release.pub) in this repo. To verify before
installing:

```bash
minisign -Vm SHA256SUMS -p zy-release.pub   # confirms the maintainer signature
...
Every release also ships a SHA-256 manifest signed with the project's Ed25519 key, fingerprint `D2429030802BE314`. The public key lives at [zy-release.pub](zy-release.pub) in this repo. The `minisign + sha256sum -c` verification recipe is in the **Verify the download first?** section above and in each release's notes.
</details>
...
The wizard asks at most six things, and skips any that have no work to do:
...
| **Enable rich previews** | Only if any of `chafa`, `poppler-utils`, `ffmpeg`, `ghostscript` is missing | Queues an `apt install` of just the missing ones, so `explore` gets image, PDF, video, and PostScript thumbnails. Text-only preview if you skip |
...
| [Nerd Fonts](https://github.com/ryanoasis/nerd-fonts) | MIT (project) and SIL OFL 1.1 (font files) | The packaging that bundles Powerline, Devicons, and Material icons into common monospace fonts. The optional font helper (`/usr/share/zy/setup-font.sh`) downloads MesloLGL from the upstream releases page at install time. zy does not bundle font files in its own package. |
...
| [stb_image](https://github.com/nothings/stb) | Public domain | PNG / JPEG / GIF / BMP decoders used by the image preview path. Bundled under `zy_src/vendor/stb/`. |
...
| [libarchive](https://libarchive.org) | BSD-2-Clause | Optional at build time, enabled in the shipped `.deb`. Powers in-place archive browsing and creation in `explore`. |
