...
Download the latest `.deb` for your architecture from [Releases](https://github.com/iskandarputra/zyshell/releases) and run:
...
That is the whole install. zy takes it from there.

### Sit and relax: the setup wizard

The moment `dpkg` finishes, zy runs an interactive setup wizard in the same terminal. Press Enter on each question to accept the recommended default; zy handles the work. Skipped steps are remembered, and re-running `dpkg -i` later picks up only the ones that still apply.

The wizard asks at most five questions, and skips any that have no work to do:
